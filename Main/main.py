...
for epoch in range(num_epochs):
    ...
    for file, df in train_dataframes.items():
        # Convert spectrogram data from strings to numeric arrays
        spectrogram_data = df['Spectrogram'].apply(lambda x: np.fromstring(x[1:-1], sep=','))
        # Stack spectrogram data into a single numpy array
        inputs = np.vstack(spectrogram_data)
        # Convert to PyTorch tensor without adding an extra channel dimension
        ...
        targets = torch.tensor(df['onset'].values)       # Assuming 'onset' contains the target values
